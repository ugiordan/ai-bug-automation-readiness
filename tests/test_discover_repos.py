...
class TestDiscoverRepos(unittest.TestCase):
    ...
    def _make_flat_layout(self, repo_names: list[str]) -> str:
    ...
    def _make_nested_layout(self, org_repos: dict[str, list[str]]) -> str:
    ...
    def test_flat_layout(self) -> None:
        """Verify flat layout repos are discovered correctly."""
    ...
    def test_nested_layout(self) -> None:
        """Verify nested org/repo layout is discovered correctly."""
    ...
    def test_flat_exclude(self) -> None:
        """Verify excluded repos are filtered in flat layout."""
    ...
    def test_nested_exclude_bare_name(self) -> None:
        """Verify bare repo name exclusion works in nested layout."""
        tmpdir = self._make_nested_layout({"org1": ["repo-a", "skip-me"]})
        results = discover_repos(tmpdir, {"skip-me"})
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0][1], "repo-a")

    def test_nested_exclude_qualified_name(self) -> None:
        """Verify qualified org/repo exclusion works in nested layout."""
    ...
    def test_empty_dir(self) -> None:
        """Verify empty directory returns no repos."""
        tmpdir = tempfile.mkdtemp()
        results = discover_repos(tmpdir, set())
        self.assertEqual(results, [])

    def test_ignores_files(self) -> None:
        """Verify non-directory files are ignored during discovery."""
    ...
    def test_mixed_layout_handles_both(self) -> None:
        """Verify mixed flat and nested layouts are both discovered."""
        tmpdir = tempfile.mkdtemp()
