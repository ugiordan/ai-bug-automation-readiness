...
def check_agent_context(repo_path):
...
def check_bug_template(repo_path):
        ...
        if full_path.exists():
            ...
            if tp.endswith(".md"):
                ...
                if frontmatter:
                    ...
                    has_bug_ref = re.search(r'\bbug\b|\bissue\b|\bproblem\b|\bdefect\b', fm_text)
                    if not has_bug_ref:
                        is_bug_template = False
                        evidence.append(f"Found {tp} but frontmatter indicates it is not a bug template")
...
def check_structured_logging(repo_path):
    ...
    source_files = find_files(repo_path, "*.go") + find_files(repo_path, "*.py") + find_files(repo_path, "*.ts")
    source_files = [f for f in source_files if not is_generated_file(f)]
    sample = random.sample(source_files, min(50, len(source_files))) if source_files else []
...
def check_code_navigability(repo_path):
    """Check file sizes, excluding generated files."""
    source_exts = {".go", ".py", ".js", ".ts", ".tsx", ".jsx", ".java", ".rs", ".rb", ".cpp", ".c", ".h"}
    files = [f for f in find_files(repo_path, "*")
...
def check_generated_code(repo_path):
    """Check what percentage of source code is auto-generated."""
    source_exts = {".go", ".py", ".js", ".ts", ".tsx", ".jsx", ".java", ".rs"}
    all_source = [f for f in find_files(repo_path, "*") if f.is_file() and f.suffix.lower() in source_exts]
...
def check_test_ratio(repo_path):
    """Check ratio of test files to source files."""
    source_exts = {".go", ".py", ".js", ".ts", ".tsx", ".jsx", ".java", ".rs"}
    test_patterns = ["_test.", "test_", "_spec.", ".spec.", ".test.", ".cy."]
    ...
    for f in find_files(repo_path, "*"):
        if f.is_file() and f.suffix.lower() in source_exts and not is_generated_file(f):
            name = f.name.lower()
            in_test_dir = "__tests__" in f.parts or "tests" in f.parts or "test" in f.parts
            test_helper_patterns = ["conftest", "helper", "utils", "util", "fixture", "factory", "common", "setup", "base"]
            is_helper = any(name.startswith(hp) or f"_{hp}" in name for hp in test_helper_patterns)
            if any(tp in name for tp in test_patterns) or (in_test_dir and not is_helper):
                test_files.append(f)
            else:
...
def check_test_execution(repo_path):
    ...
    for prefix, makefile in makefiles:
        ...
        test_targets = re.findall(
            r'^(test|unittest|unit-test|unit_test|e2e-test|e2e_test|'
            r'integration-test|functest)\s*:',
...
def check_coverage_config(repo_path):
    ...
    if ci_dir.exists():
        for cf in list(ci_dir.glob("*.yml")) + list(ci_dir.glob("*.yaml")):
            ...
            if re.search(r'\bcoverag(e|eprofile)\b|\bcoverprofil\b|\bcodecov\b', content, re.IGNORECASE):
                score = max(score, 60)
...
def check_ci_runs_tests(repo_path):
    ...
    test_patterns = [
        ...
        r'mvn\s+test', r'cargo\s+test', r'tox\b', r'nox\b',
    ]

    for cf in ci_files:
        content = read_file_safe(cf)
        file_has_pr = bool(re.search(r'pull_request(?:_target)?:', content))
        file_has_test = False
        if file_has_pr:
            has_pr_trigger = True
...
def check_test_isolation(repo_path):
    """Check if tests can run without external infrastructure."""
    source_exts = {".go", ".py", ".js", ".ts", ".tsx", ".jsx", ".java"}
    test_patterns_names = ["_test.", "test_", "_spec.", ".spec.", ".test.", ".cy."]

    test_files = []
    for f in find_files(repo_path, "*"):
        if f.is_file() and f.suffix.lower() in source_exts:
            name = f.name.lower()
            if any(tp in name for tp in test_patterns_names):
                test_files.append(f)

    if not test_files:
        return 0, ["No test files found"]

    sample = random.sample(test_files, min(30, len(test_files)))
    infra_patterns = [
...
def check_pr_template(repo_path):
...
def check_linting_in_ci(repo_path):
...
def check_contributing_guide(repo_path):
